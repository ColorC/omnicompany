...
OMNI-013 HIGH: 绕过 omnicompany.core.guarded_write.write_file 直接用 Path.write_text /
  open(..,"w") 写文件。注意豁免：
    - src/omnicompany/core/guarded_write.py / omnimark.py 自身（唯一裸写入口）
...
OMNI-PIPELINE-IF (你判断): pipeline 文件（packages/<domain>/pipeline.py / run.py / routers.py）
  是否通过标准 OmniCompany 接口构建？标准接口包括:
    - Router 子类必须继承 omnicompany.runtime.routing.router.Router (这是当前唯一的 Router 定义位置)
    - Format 必须构造自 omnicompany.protocol.format.Format
    - pipeline.py 必须返回 omnicompany.protocol.pipeline.PipelineSpec
...
- 任何试图绕过架构约束的创意写法
- 自造 pipeline 框架，不使用 omnicompany.protocol.* 标准接口
