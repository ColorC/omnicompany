...
**不用我**:
- 想做语义级深度检查 → 找 [doctor](../doctor/SKILL.md) (含 LLM 检查器, 慢但深)
- 想扫源码静态合规 → 找 [guardian](../../_core/guardian/SKILL.md)
- 想测框架本身能跑 → 找 [selftest](../../_core/selftest/SKILL.md)
...
```bash
omni run pipeline_ci -i scope="packages/domains/gameplay_system"
```
...
```python
from omnicompany.packages.services._diagnosis.pipeline_ci.pipeline import build_pipeline
...
| 想看 ErrorRouteAuditor 在做什么 | 它来自 workflow_factory (跨 Team 直调, 局限 2) | 看 [../../_core/workflow_factory/](../../_core/workflow_factory/) 里 ErrorRouteAuditor 实现 |
...
- 跟 doctor 关系 → [../doctor/SKILL.md](../doctor/SKILL.md) (语义级深度检查 vs CI gate)
- 跨 Team 依赖 ErrorRouteAuditor → [../../_core/workflow_factory/](../../_core/workflow_factory/)
