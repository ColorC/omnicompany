...
# [OMNI] why="阶段 2 后续 3: 派生 agent 也应有出口检查工具. 拒数字打分铁律仍适用. 校验派生过程产物完整 (hypothesis_ids + 来源 + 派生 creative_content)"
...
校验内容 (V0):
  ...
  - creative_content (派生的整体说明: 用户铁律 - 拒打分要来龙去脉)
...
class SubmitDerivationReportRouter(SingleToolRouter):
    ...
    DESCRIPTION: ClassVar[str] = (
        ...
        "creative_content (overall commentary: 来龙去脉 - 派生时考虑了什么 / 派生策略 / 哪些是 hard rule 候选 / "
        "哪些是软语义 / 跟现有假设库的关系).\n"
    ...
    INPUT_SCHEMA: ClassVar[dict] = {
        ...
        "required": ["source_paths", "derived_hypothesis_ids", "creative_content"],
    }
    ...
    def _execute(self, args: dict, ctx: ToolContext) -> str:
        ...
        if banned_present:
            raise ToolExecutionError(
                ...
                f"Encode same information as natural-language creative_content. Re-submit without {banned_present}."
            )
        ...
        logger.info(
            "[submit_derivation_report] OK sources=%d derived=%d creative_content_len=%d",
            len(source_paths), len(derived), len(creative_content),
        )

        return (
            f"Derivation report accepted. {len(derived)} hypothesis(es) derived from "
            f"{len(source_paths)} source(s), creative_content {len(creative_content)} chars. "
