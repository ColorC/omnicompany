...
> 给 AI agent 路径拼接错误产生的"错位嵌套垃圾目录" (例 `E:\e\workspace`) 做扫描 + LLM 判断 + PowerShell 清理脚本生成. 三节点链路 (扫描 → LLM 判断 → 打印计划), **只打印不自动删除**.
...
形态: **三节点线性 Team** (扫描器 ANCHOR → 异常检测器 LLM → 计划生成器 ANCHOR), 类似 [lap_auditor](../lap_auditor/) 的 ANCHOR-LLM-ANCHOR 模式.
...
- 多 keyword 输入 (当前一次一个)
- 跟 [CORE-SELF-STABILITY 第二阶段](../../../../../../docs/plans/guardian/[2026-05-04]CORE-SELF-STABILITY/plan.md) 自我画像漂移检测有交集 — 但跟项目内代码漂移不同, cleanup_bot 看宿主机文件系统层
...
- 类似设计模式 → [../lap_auditor/README.md](../lap_auditor/README.md) (同三节点 ANCHOR-LLM-ANCHOR 链路)
