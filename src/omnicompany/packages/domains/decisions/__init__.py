...
# [OMNI] summary="decisions domain 包根。主线=决策记录(非提取):一套源无关的统一决策契约 + 统一决策库,把对话/协作平台/策划文档/札记里的决策汇成一棵可搜索的决策树。"
# [OMNI] why="用户 2026-06-17 新开「决策记录」线。LLM 把记录决策的成本打到 ROI 为正,个人决策树沉淀成重点;决策有多源多落地面,需统一契约(符号统一)。非 gameplay_system 自有内容内化进 omnicompany。"
...
边界:
  ...
  - omnicompany 是实验室+个人决策记录的收集管理处;管线成熟后可导出成
    claude code 标准 workflow js,落进对应执行端工程(如 gameplay_system),且执行端绝不反向依赖本仓。

已落地:schema 三件套(formats.py)+ 统一库(library.py)+ 索引召回(catalog.py)+ CLI(omni decisions)。
待加:抽取管线(对话源先行)/ 协作平台消息源 / gameplay_system 落地面渲染(见 DESIGN.md)。
"""
