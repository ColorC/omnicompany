...
function userTextFingerprint(m: NormalizedMessage): string | null {
  ...
  const t = (m.content || '')
    .trim()
    .replace(/^(ultrathink|think harder|think hard|think):\s*/i, '')
    .trim();
...
function sortMessagesByTimestamp(messages: NormalizedMessage[]): NormalizedMessage[] {
  return [...messages].sort(compareMessagesByTimestamp);
}

function computeMerged(server: NormalizedMessage[], realtime: NormalizedMessage[]): NormalizedMessage[] {
  if (realtime.length === 0) return server;
  if (server.length === 0) return dedupeAdjacentAssistantEchoes(sortMessagesByTimestamp(realtime));
  ...
  const extra = realtime.filter((m) => {
    ...
    // 防御: 某些 realtime 帧(如总控被事件注入的消息)可能没有 id —— filter 不能因一条畸形消息整体崩,
    // 否则 SessionStore.fetch 失败、会话永远加载不出、composer 无 session 可发(发送彻底失效)。
    ...
    if (m.kind === 'text' && m.role === 'assistant') {
      ...
      if (text && serverAssistantTexts.has(text)) return false;
    }
    return true;
  });
  if (extra.length === 0) return server;
  return dedupeAdjacentAssistantEchoes(sortMessagesByTimestamp([...server, ...extra]));
}

export const sessionStoreInternalsForTest = {
  computeMerged,
};

function mergeRealtimeMessages(existing: NormalizedMessage[], incoming: NormalizedMessage[]): NormalizedMessage[] {
...
const MAX_REALTIME_MESSAGES = 10_000;
...
export function useSessionStore() {
  ...
  const appendRealtime = useCallback((sessionId: string, msg: NormalizedMessage) => {
    ...
    const normalizedMessage =
      msg.sessionId === resolvedSessionId
        ? msg
        : { ...msg, sessionId: resolvedSessionId };
    let updated = mergeRealtimeMessages(slot.realtimeMessages, [normalizedMessage]);
  ...
  const appendRealtimeBatch = useCallback((sessionId: string, msgs: NormalizedMessage[]) => {
    ...
    let updated = mergeRealtimeMessages(slot.realtimeMessages, normalizedMessages);
    if (updated.length > MAX_REALTIME_MESSAGES) {
      updated = updated.slice(-MAX_REALTIME_MESSAGES);
    }
    slot.realtimeMessages = updated;
