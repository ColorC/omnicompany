...
interface UseChatComposerStateArgs {
  ...
  /** 本地(前端动作)斜杠命令: 进菜单 + 命中走 onLocalCommand, 不发后端/LLM。总控 /compact /new 用。 */
...
export function useChatComposerState({
  ...
  // 默认不拼思考暗号词(none): "默认最大思考"改由后端 claude provider 的 SDK effort='max'
  // 干净实现, 不再往消息前拼 "ultrathink:" 污染正文。选择器仍可手动临时加暗号(仅 claude 生效)。
    ...
    (result: CommandExecutionResult) => {
      ...
      switch (action) {
        ...
        case 'goal':
          addMessage({
            type: 'assistant',
            content: data.message || 'Goal command completed.',
            timestamp: Date.now(),
          });
          break;
  ...
  const executeCommand = useCallback(
    async (command: SlashCommand, rawInput?: string) => {
      ...
      // 本地(前端动作)命令: 不发后端/LLM, 直接交回调(总控 /compact /new)。两条路径(打字提交 + 菜单选中)
      // 都汇到这里, 所以放这一处即可。
      if ((command as Record<string, unknown>).local) {
        try { onLocalCommand?.(command.name); } catch { /* ignore */ }
        setInput('');
  ...
  const isNativePassthroughCommand = useCallback((command: SlashCommand | null | undefined) => {
    return Boolean(command?.metadata && (command.metadata as Record<string, unknown>).nativePassthrough);
  }, []);
  ...
  const handleSubmit = useCallback(
    async (
      ...
      const submitKey = `${thinkingMode}\n${currentInput.trim()}`;
      const now = Date.now();
      if (lastSubmitRef.current?.key === submitKey && now - lastSubmitRef.current.at < 800) {
        return;
      }
      ...
      if (trimmedInput.startsWith('/')) {
        ...
        const matchedCommand = slashCommands.find(
          (cmd: SlashCommand) => cmd.name === commandName || cmd.name === bareCommandName || `/${cmd.name}` === commandName,
        );
        if (matchedCommand) {
          if (isNativePassthroughCommand(matchedCommand)) {
            resetCommandMenuState();
          } else {
      ...
      // think/think hard/.../ultrathink 是 Claude Code 专属的"思考预算暗号词"(写进 prompt 本身
      // 就是开关), 只对 claude 生效。codex/gemini 不认这词, 拼上去纯属噪声 + 污染消息, 故只对 claude 拼。
      const selectedThinkingMode = thinkingModes.find((mode: { id: string; prefix?: string }) => mode.id === thinkingMode);
      if (selectedThinkingMode && selectedThinkingMode.prefix && provider === 'claude') {
      ...
      setImageErrors(new Map());
      setIsTextareaExpanded(false);
      setThinkingMode('none');  // 思考拉满由后端 effort='max' 负责, 前端不拼暗号词
