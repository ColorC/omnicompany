...
interface UseChatRealtimeHandlersArgs {
  latestMessage: LatestChatMessage | LatestChatMessage[] | null;
...
export function useChatRealtimeHandlers({
  ...
  const lastProcessedMessageRef = useRef<LatestChatMessage | LatestChatMessage[] | null>(null);
  ...
  useEffect(() => {
    ...
    const incomingMessages = Array.isArray(latestMessage) ? latestMessage : [latestMessage];
    ...
    if (!msg.kind) {
      ...
      switch (messageType) {
        ...
        case 'pending-permissions-response': {
          ...
          if (permSessionId && !isCurrentPermSession) continue;
    ...
    switch (msg.kind) {
      ...
      case 'permission_request': {
        ...
        onSessionAwaitingPermission?.(sid);
        break;
      }
      ...
      case 'status': {
        ...
        } else if (msg.text && msg.canInterrupt === false) {
          // canInterrupt:false = 空闲通知(如元数据 ack / 目标暂停), 不是在途 turn。
          // 只更新状态文字, 绝不 setIsLoading(true) —— 否则没有后续 complete 会卡死 loading。
          setClaudeStatus({ text: msg.text, tokens: msg.tokens || 0, can_interrupt: false });
        } else if (msg.text) {
          setClaudeStatus({
            text: msg.text,
            tokens: msg.tokens || 0,
