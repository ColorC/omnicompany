...
interface UseSlashCommandsOptions {
  ...
  /** 本地(前端动作)命令: 与后端命令一起进斜杠菜单。带 local:true, 执行走 onLocalCommand。
   *  总控用它注册 /compact /new(见 entities/controller)。 */
...
const isNativePassthrough = (command: SlashCommand) =>
  Boolean(command.metadata && (command.metadata as Record<string, unknown>).nativePassthrough);
...
export function useSlashCommands({
  ...
  useEffect(() => {
    const fetchCommands = async () => {
      if (!selectedProject) {
        setSlashCommands(localCommands ? [...localCommands] : []);
        setFilteredCommands([]);
        return;
      }
      ...
      try {
        ...
        // 本地命令(总控 /compact /new 等前端动作)排在后端命令前面, 一起进菜单。
        setSlashCommands([...(localCommands || []), ...sortedCommands]);
      } catch (error) {
    ...
    fetchCommands();
  }, [selectedProject, provider, localCommands]);
  ...
  const selectCommandFromKeyboard = useCallback(
    (command: SlashCommand) => {
      ...
      const commandText = isNativePassthrough(command) ? `/${command.name}` : command.name;
      const newInput = `${textBeforeSlash}${commandText} ${textAfterQuery}`;

      setInput(newInput);
      resetCommandMenuState();

      if (isNativePassthrough(command)) {
        return;
      }
  ...
  const handleCommandSelect = useCallback(
    (command: SlashCommand | null, index: number, isHover: boolean) => {
      ...
      if (isNativePassthrough(command)) {
        ...
        setInput((current) => {
          const textBeforeSlash = current.slice(0, slashPosition >= 0 ? slashPosition : current.length);
          const textAfterSlash = current.slice(slashPosition >= 0 ? slashPosition : current.length);
          const spaceIndex = textAfterSlash.indexOf(' ');
          const textAfterQuery = spaceIndex !== -1 ? textAfterSlash.slice(spaceIndex) : ' ';
          return `${textBeforeSlash}${commandText}${textAfterQuery}`;
        });
        resetCommandMenuState();
    ...
    [selectedProject, slashPosition, setInput, textareaRef, trackCommandUsage, onExecuteCommand, resetCommandMenuState],
  );
