      ...
      <div className="relative overflow-hidden rounded-2xl border border-gray-200/80 bg-white shadow-lg dark:border-gray-700/50 dark:bg-gray-800/90 dark:shadow-2xl">
        ...
        <div className="px-4 pb-2 pt-3.5">
          <div className="mb-1.5 flex items-center gap-2.5">
            ...
            <div className="flex min-w-0 flex-1 items-center gap-2">
              <span className="text-[14px] font-medium uppercase tracking-wide text-gray-400 dark:text-gray-500">
                Claude needs your input
              </span>
              {q.header && (
                <span className="inline-flex items-center rounded border border-blue-100 bg-blue-50 px-1.5 py-px text-[14px] font-semibold uppercase tracking-wider text-blue-600 dark:border-blue-800/50 dark:bg-blue-900/30 dark:text-blue-400">
                  {q.header}
                </span>
              )}
            </div>
            ...
            {/* Step counter */}
            {!isSingle && (
              <span className="flex-shrink-0 text-[14px] tabular-nums text-gray-400 dark:text-gray-500">
                {currentStep + 1}/{total}
              </span>
            )}
          </div>
          ...
          {multi && (
            <span className="text-[14px] text-gray-400 dark:text-gray-500">Select all that apply</span>
          )}
        </div>
          ...
          <div className="space-y-1">
            {q.options.map((opt, optIdx) => {
              ...
              return (
                ...
                >
                  {/* Keyboard hint */}
                  <kbd className={`flex h-5 w-5 flex-shrink-0 items-center justify-center rounded font-mono text-[14px] transition-all duration-150 ${
                    isSelected
              ...
              <div className="pl-[30px] pr-0.5">
                <div className="relative">
                  <input
                    ...
                    className="w-full rounded-lg border-0 bg-gray-50 px-3 py-1.5 text-[15px] text-gray-900 outline-none ring-1 ring-gray-200 transition-shadow duration-200 placeholder:text-gray-400 focus:ring-2 focus:ring-blue-400 dark:bg-gray-900/60 dark:text-gray-100 dark:ring-gray-700 dark:placeholder:text-gray-600 dark:focus:ring-blue-500"
                  />
                  <kbd className="absolute right-2 top-1/2 -translate-y-1/2 rounded border border-gray-200 bg-gray-100 px-1 py-0.5 font-mono text-[14px] text-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-600">
                    Enter
                  </kbd>
                </div>
              </div>
        ...
        <div className="flex items-center justify-between gap-2 border-t border-gray-100 bg-gray-50/50 px-4 py-2 dark:border-gray-700/50 dark:bg-gray-800/50">
          <button
            type="button"
            onClick={handleSkip}
            className="text-[14px] text-gray-400 transition-colors hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300"
          >
            {isSingle ? 'Skip' : 'Skip all'}
            <span className="ml-1 text-[14px] text-gray-300 dark:text-gray-600">Esc</span>
          </button>
          ...
          <div className="flex items-center gap-1.5">
            {!isSingle && !isFirst && (
              <button
                ...
                className="inline-flex items-center gap-0.5 rounded-lg px-2.5 py-1.5 text-[14px] font-medium text-gray-600 transition-all duration-150 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700/60"
              >
            ...
            {isLast ? (
              <button
                ...
                className="inline-flex items-center gap-1 rounded-lg bg-gradient-to-r from-blue-600 to-blue-500 px-3.5 py-1.5 text-[14px] font-semibold text-white shadow-sm transition-all duration-200 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-30 disabled:shadow-none dark:from-blue-500 dark:to-blue-600"
              >
                Submit
                <span className="ml-0.5 font-mono text-[14px] opacity-70">Enter</span>
              </button>
            ) : (
              <button
                type="button"
                onClick={() => setCurrentStep(s => s + 1)}
                className="inline-flex items-center gap-1 rounded-lg bg-gradient-to-r from-blue-600 to-blue-500 px-3.5 py-1.5 text-[14px] font-semibold text-white shadow-sm transition-all duration-200 hover:shadow-md dark:from-blue-500 dark:to-blue-600"
              >
                Next
                <span className="ml-0.5 font-mono text-[14px] opacity-70">Enter</span>
              </button>
