...
export const FileListContent: React.FC<FileListContentProps> = ({
  ...
  return (
    <div>
      {title && (
        <div className="mb-1 text-[14px] text-gray-500 dark:text-gray-400">
          {title}
        </div>
      )}
        ...
        {files.map((file, index) => {
          ...
          return (
            <span key={index} className="inline-flex items-center">
              <button
                onClick={handleClick}
                className="font-mono text-[14px] text-blue-600 transition-colors hover:text-blue-700 hover:underline dark:text-blue-400 dark:hover:text-blue-300"
                title={filePath}
              >
                {fileName}
              </button>
              {index < files.length - 1 && (
                <span className="ml-1 text-[14px] text-gray-300 dark:text-gray-600">,</span>
              )}
            </span>
          );
        })}
