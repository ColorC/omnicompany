...
export const TOOL_CONFIGS: Record<string, ToolDisplayConfig> = {
  ...
  Edit: {
    input: {
      type: 'collapsible',
      title: (input) => {
        const filePath = input.file_path || input.changes?.[0]?.path || input.changes?.[0]?.file_path || '';
      ...
      getContentProps: (input) => ({
        ...
        newContent: input.new_string || JSON.stringify(input.changes || input, null, 2),
        filePath: input.file_path || input.changes?.[0]?.path || input.changes?.[0]?.file_path,
        badge: 'Edit',
        badgeColor: 'gray'
      })
    },
...
export function getToolConfig(toolName: string): ToolDisplayConfig {
  ...
  return TOOL_CONFIGS[toolName] || TOOL_CONFIGS[aliases[String(toolName || '').toLowerCase()]] || TOOL_CONFIGS.Default;
}

/**
 * Tool results must stay inspectable in Omni dogfood. Upstream marks some
 * successful results hidden to reduce noise; here they render collapsed.
 */
export function shouldHideToolResult(_toolName: string, _toolResult: any): boolean {
  return false;
}

/**
 * Results upstream would hide are instead collapsed by default.
 */
export function shouldCollapseToolResult(toolName: string, toolResult: any): boolean {
