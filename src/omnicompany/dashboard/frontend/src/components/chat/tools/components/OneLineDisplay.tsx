...
export const OneLineDisplay: React.FC<OneLineDisplayProps> = ({
      ...
      <div className="group my-1">
        <div className="flex items-start gap-2">
          ...
          <div className="flex min-w-0 flex-1 items-start gap-2">
            <div className="min-w-0 flex-1 rounded bg-gray-900 px-2.5 py-1 dark:bg-black">
              <code className={`font-mono text-sm text-green-400 ${wrapText ? 'whitespace-pre-wrap break-all' : 'block truncate'}`}>
        ...
        {secondary && (
          <div className="ml-7 mt-1">
            <span className="text-[14px] italic text-muted-foreground/60">
              {secondary}
            </span>
          </div>
        )}
  ...
  if (action === 'open-file') {
    ...
    return (
      <div className={`group flex items-center gap-1.5 border-l-2 ${colorScheme.border} my-0.5 py-0.5 pl-3`}>
        <span className="flex-shrink-0 text-sm text-muted-foreground">{label || toolName}</span>
  ...
  if (action === 'jump-to-results') {
    return (
      <div className={`group flex items-center gap-1.5 border-l-2 ${colorScheme.border} my-0.5 py-0.5 pl-3`}>
        <span className="flex-shrink-0 text-sm text-muted-foreground">{label || toolName}</span>
        <span className="text-[14px] text-muted-foreground/40">/</span>
        <span className={`min-w-0 flex-1 truncate font-mono text-sm ${colorScheme.primary}`}>
          {value}
        </span>
        {secondary && (
          <span className="flex-shrink-0 text-[14px] italic text-muted-foreground/60">
            {secondary}
          </span>
        )}
        ...
        {toolResult && (
          <a
            ...
            className="flex flex-shrink-0 items-center gap-0.5 text-[14px] text-primary transition-colors hover:text-primary/80"
          >
            <svg className="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className={`group flex items-center gap-1.5 ${colorScheme.background || ''} border-l-2 ${colorScheme.border} my-0.5 py-0.5 pl-3`}>
      {icon && icon !== 'terminal' && (
        <span className={`${colorScheme.icon} flex-shrink-0 text-sm`}>{icon}</span>
      )}
      {!icon && (label || toolName) && (
        <span className="flex-shrink-0 text-sm text-muted-foreground">{label || toolName}</span>
      )}
      ...
      <span className={`font-mono text-sm ${wrapText ? 'whitespace-pre-wrap break-all' : 'truncate'} min-w-0 flex-1 ${colorScheme.primary}`}>
        {value}
      </span>
      {secondary && (
        <span className={`text-[14px] ${colorScheme.secondary} flex-shrink-0 italic`}>
          {secondary}
        </span>
      )}
      {status && <ToolStatusBadge status={status} />}
