...
export const TaskListContent: React.FC<TaskListContentProps> = ({ content }) => {
  ...
  if (tasks.length === 0) {
    return (
      <pre className="whitespace-pre-wrap font-mono text-[14px] text-gray-600 dark:text-gray-400">
        {content}
      </pre>
    );
  }
        ...
        {tasks.map((task) => {
          ...
          return (
            ...
            >
              ...
              <span className="flex-shrink-0 font-mono text-[14px] text-gray-400 dark:text-gray-500">
                #{task.id}
              </span>
              <span className={`flex-1 truncate text-sm ${config.textClass}`}>
                {task.subject}
              </span>
              <span className={`flex-shrink-0 rounded border px-1 py-px text-[14px] ${config.badgeClass}`}>
