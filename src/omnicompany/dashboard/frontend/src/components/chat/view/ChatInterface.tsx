...
function ChatInterface({
  ...
  useEffect(() => {
    if (!selectedSession?.id || !onSessionPreview) return;
    for (let i = visibleMessages.length - 1; i >= 0; i -= 1) {
      const msg = visibleMessages[i];
      if (msg.isThinking || msg.isToolUse || msg.isInteractivePrompt || msg.isTaskNotification) {
        continue;
      }
      const content = typeof msg.content === 'string' ? msg.content.trim().replace(/\s+/g, ' ') : '';
      if (!content) continue;
      const preview = content.slice(0, 30);
      const key = `${selectedSession.id}:${preview}`;
      if (key !== lastPreviewRef.current) {
        lastPreviewRef.current = key;
        onSessionPreview(selectedSession.id, preview);
      }
      break;
    }
  }, [onSessionPreview, selectedSession?.id, visibleMessages]);
