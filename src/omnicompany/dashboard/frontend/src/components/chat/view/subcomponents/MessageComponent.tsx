...
import { ToolRenderer, shouldHideToolResult, shouldCollapseToolResult } from '../../tools';
...
const MessageComponent = memo(({ message, prevMessage, createDiff, onFileOpen, onShowSettings, onGrantToolPermission, autoExpandTools, showRawParameters, showThinking, selectedProject, provider }: MessageComponentProps) => {
  ...
  const isGrouped = prevMessage && prevMessage.type === message.type &&
    Boolean(prevMessage.isThinking) === Boolean(message.isThinking) &&
    Boolean(prevMessage.isToolUse) === Boolean(message.isToolUse) &&
  ...
  const shouldShowAssistantCopyControl = message.type === 'assistant' &&
    ...
    !message.isThinking;
  const contextPayload = message.contextPayload as any;
  const contextItems = Array.isArray(contextPayload?.contexts) ? contextPayload.contexts.slice(0, 6) : [];
  const contextStatus = String(message.contextStatus || '');
  const isGoalNotification = contextStatus.startsWith('goal') || contextPayload?.kind === 'goal';
  const notificationLabel = isGoalNotification ? 'Omni goal' : 'Omni context';
  const notificationClass = isGoalNotification
    ? 'border-emerald-800/50 bg-emerald-950/20 text-emerald-100'
    : 'border-cyan-800/50 bg-cyan-950/20 text-cyan-100';
  const notificationTitleClass = isGoalNotification ? 'text-emerald-200' : 'text-cyan-200';
  const notificationDotClass = isGoalNotification ? 'bg-emerald-300' : 'bg-cyan-300';
  const notificationTextClass = isGoalNotification ? 'text-emerald-100/90' : 'text-cyan-100/90';
  ...
  return (
    ...
    >
      {message.type === 'user' ? (
        ...
        <div className="flex w-full items-end space-x-0 sm:w-auto sm:max-w-[85%] sm:space-x-3 md:max-w-md lg:max-w-lg xl:max-w-xl">
          <div className="group flex-1 rounded-2xl rounded-br-md bg-blue-600 px-3 py-2 text-white shadow-sm sm:flex-initial sm:px-4">
            ...
            <div className="mt-1 flex items-center justify-end gap-1 text-sm text-blue-100">
      ...
      ) : message.isContextNotification ? (
        <div className="w-full px-3 sm:px-0" data-context-notification>
          <div className={`rounded-md border px-3 py-2 text-sm ${notificationClass}`}>
            <div className={`mb-1 flex items-center gap-2 font-medium ${notificationTitleClass}`}>
              <span className={`inline-block h-1.5 w-1.5 rounded-full ${notificationDotClass}`} />
              <span>{notificationLabel}</span>
              {!isGoalNotification && contextPayload?.total != null && (
                <span className="text-cyan-400/80">{contextPayload.total} paths</span>
              )}
              {contextPayload?.missing_total ? (
                <span className="text-amber-300">{contextPayload.missing_total} missing</span>
              ) : null}
            </div>
            <div className={notificationTextClass}>{message.content}</div>
            {contextItems.length > 0 && (
              <div className="mt-2 space-y-1 border-t border-cyan-900/60 pt-2">
                {contextItems.map((item: any, index: number) => (
                  <button
                    key={`${item.path || index}`}
                    type="button"
                    onClick={() => onFileOpen?.(String(item.abs_path || item.path || ''))}
                    className="block max-w-full truncate text-left font-mono text-[14px] text-cyan-300 hover:text-cyan-100"
        ...
        <div className="w-full">
          ...
          <div className="w-full">
            ...
            {message.isToolUse ? (
              <>
                <div className="flex flex-col">
                  <div className="flex flex-col">
                    <Markdown className="prose prose-sm max-w-none dark:prose-invert" onFileOpen={onFileOpen} projectPath={selectedProject?.fullPath || selectedProject?.path}>
                  ...
                  message.toolResult.isError ? (
                    ...
                    >
                      <div className="relative mb-2 flex items-center gap-1.5">
                        ...
                        <span className="text-sm font-medium text-red-700 dark:text-red-300">{t('messageTypes.error')}</span>
                      </div>
                      <div className="relative text-sm text-red-900 dark:text-red-100">
                        <Markdown className="prose prose-sm prose-red max-w-none dark:prose-invert" onFileOpen={onFileOpen} projectPath={selectedProject?.fullPath || selectedProject?.path}>
                          ...
                          <div className="mt-4 border-t border-red-200/60 pt-3 dark:border-red-800/60">
                            <div className="flex flex-wrap items-center gap-2">
                              <button
                                ...
                                className={`inline-flex items-center gap-2 rounded-md border px-3 py-1.5 text-sm font-medium transition-colors ${permissionSuggestion.isAllowed || permissionGrantState === 'granted'
                              ...
                              {onShowSettings && (
                                <button
                                  ...
                                  className="text-sm text-red-700 underline hover:text-red-800 dark:text-red-200 dark:hover:text-red-100"
                                >
                                  {t('permissions.openSettings')}
                                </button>
                              )}
                            </div>
                            <div className="mt-2 text-sm text-red-700/90 dark:text-red-200/80">
                    ...
                    shouldCollapseToolResult(message.toolName || 'UnknownTool', message.toolResult) ? (
                      <details
                        id={`tool-result-${message.toolId}`}
                        className="group/result mt-1 scroll-mt-4 rounded border border-gray-200/40 dark:border-gray-700/40"
                      >
                        <summary className="flex cursor-pointer select-none items-center gap-1.5 px-2 py-1 text-[14px] text-muted-foreground hover:text-foreground">
                          <svg
                            className="h-3 w-3 flex-shrink-0 transition-transform duration-150 group-open/result:rotate-90"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
                          ...
                          <span className="font-medium">{message.toolName || 'Tool'} result</span>
                          <span className="text-muted-foreground/60">
                            {(() => {
                              const content = message.toolResult?.content;
                              const len = typeof content === 'string' ? content.length : (content ? JSON.stringify(content).length : 0);
                              return `- ${len} bytes`;
                            })()}
                          </span>
                        </summary>
                        <div className="border-t border-gray-200/30 px-2 py-1.5 dark:border-gray-700/30">
                          <ToolRenderer
                            toolName={message.toolName || 'UnknownTool'}
                            toolInput={message.toolInput}
                            toolResult={message.toolResult}
              ...
              <Reasoning defaultOpen={false}>
                <ReasoningTrigger />
                <ReasoningContent>
                  <Markdown className="prose prose-sm prose-gray max-w-none dark:prose-invert" onFileOpen={onFileOpen} projectPath={selectedProject?.fullPath || selectedProject?.path}>
                    {message.content}
                  </Markdown>
            ...
            {(shouldShowAssistantCopyControl || !isGrouped) && (
              <div className="mt-1 flex w-full items-center gap-2 text-[14px] text-gray-400 dark:text-gray-500">
