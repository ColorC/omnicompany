...
function defaultContextWindow(provider: Provider | string): number {
  if (provider === 'codex') return 400000;
  const envValue = parseInt(import.meta.env.VITE_CONTEXT_WINDOW || '', 10);
  return Number.isFinite(envValue) && envValue > 0 ? envValue : 200000;
}
          ...
          <div className="absolute bottom-full left-0 right-0 z-50 mb-2 max-h-48 overflow-y-auto rounded-xl border border-border/50 bg-card/95 shadow-lg backdrop-blur-md">
            {filteredFiles.map((file, index) => (
              ...
              >
                ...
                <div className="font-mono text-sm text-muted-foreground">{file.path}</div>
              </div>
            ))}
          </div>
        )}
        ...
        <PromptInputFooter>
          <PromptInputTools>
            ...
            <button
              ...
              className={`rounded-lg border p-2 text-sm font-medium transition-all duration-200 sm:px-2.5 sm:py-1 ${
            ...
            <TokenUsagePie used={tokenBudget?.used || 0} total={tokenBudget?.total || defaultContextWindow(provider)} />
            ...
            >
              ...
              {slashCommandsCount > 0 && (
                <span
                  className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-primary text-[14px] font-bold text-primary-foreground"
                >
                  {slashCommandsCount}
                </span>
              )}
          ...
          <div className="flex items-center gap-2">
            <div
              className={`hidden text-sm text-muted-foreground/50 transition-opacity duration-200 lg:block ${
