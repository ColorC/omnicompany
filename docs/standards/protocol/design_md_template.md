...
> **权威来源**：`self_creative_content_three_files.md`（自我叙事三件套规范，2026-05-04 立）——本文件是其 DESIGN.md 模板**细则**，冲突时以三件套规范为准
...
<!-- 例: - `PipelineRunner.run(initial_input)` — [runner.py:1174](../runtime/exec/runner.py#L1174) -->
...
> **权威范本**: [`packages/services/omnicompany/DESIGN.md`](../../src/omnicompany/packages/services/omnicompany/DESIGN.md)
> **命名**: Worker / Material / Team 是 2026-04-19 起新代码强制用的名字（见 [`terminology.md`](terminology.md)）
...
**kind 速查**（见 [`material.md`](material.md) F-16 / F-19）：
...
**Worker 粒度硬规则**（[`terminology.md`](terminology.md) §6.5）：
...
新 Team 落地验证（[`material_dispatcher.py`](../../src/omnicompany/packages/services/omnicompany/material_dispatcher.py)）：
...
- [`packages/services/guardian/workers/`](../../src/omnicompany/packages/services/guardian/workers/) — 类 A 迁移 4 Worker（GitDiffScan / RuleEngine / LLMJudge / AuditTow）
- [`packages/services/omnicompany/agent_team_demo.py`](../../src/omnicompany/packages/services/omnicompany/agent_team_demo.py) — Agent Team 4 Worker（Context / LLM / Tool / Finalizer · 含子 job 示例）
