<!-- [OMNI] origin=ai-ide ts=2026-05-10T00:00:00Z type=standard agent=ai-ide -->
<!-- [OMNI] summary="NormalizedMessage 协议规范 — 各 LLM provider 后端统一输出形态, 前端 ChatInterface 通用消费. 14 个 kind 真用到的字段定义, 跟 useChatRealtimeHandlers.ts 1:1 反推" -->
<!-- [OMNI] tags=standard,protocol,multi-provider,chat,normalized-message -->
<!-- [OMNI] material_id="material:standards.protocol.normalized_message_spec.md" -->

# NormalizedMessage 协议规范

供 `MULTIPROVIDER-CHAT-PLATFORM`
阶段 1 立, 后续接入新 provider 实现者参考.

## 1 · 设计原则

1. **后端转, 前端通用消费**: 各 provider 子类 (`ClaudeProvider` / 后续 `CodexProvider` 等)
   在 backend 把自家 SDK 原始消息转换为 NormalizedMessage. 前端 `ChatInterface`
...
3. **schema 反推自前端 hook**: kind 名跟字段从 `useChatRealtimeHandlers.ts`
...
1. **加新 kind**: 必先改 `useChatRealtimeHandlers.ts` +
...
- 前端真消费 hook: `useChatRealtimeHandlers.ts`
