...
```yaml
- id: voxel_engine.material-authoring
  applies:
    projects: [voxel_engine]
    kinds: [material, standard_md]
    path_match:
      - docs/standards/_domain_specific/voxel_engine/**
      - src/omnicompany/packages/domains/voxel_engine/**/materials.py
...
omni context resolve \
  --plan current \
  --path docs/standards/_domain_specific/voxel_engine/building.md \
  --topic "voxel_engine material standard and worker definition" \
  --json
```
...
- `src/omnicompany/packages/domains/voxel_engine/DESIGN.md`
- `src/omnicompany/packages/domains/voxel_engine/.omni/manifest.yaml`
- `src/omnicompany/packages/domains/voxel_engine/.omni/workspace.yaml`
...
| 新建 voxel_engine 专属规则但不引用全局 material/worker/template 规范 | 两套规则冲突 |
