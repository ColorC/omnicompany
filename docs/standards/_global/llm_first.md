...
**典型案例**: [`frame_correspondence_loop.py`](../../src/omnicompany/packages/domains/gameplay_system/ux/routers/frame_correspondence_loop.py)（B-Step4）在 2026-04-17 暴露了 3 个此类规则：
...
| `_pick_best_sections` 按 EP 号择新 | L180 | "EP 号越高越好" | 反例：competitive_event **event_phase seasonal_event + event_phase activity_sample_a是两期并存的活动**，被互相挤掉 |
...
跨项目铁律 [feedback_no_manual_parse_use_structured_output](memory) 立 (2026-04-25 用户立).

**手解必失败**: LLM 偶尔输出 creative_content + JSON / 单引号 / unescaped 特殊字符, regex 接不住. function call 走 API schema 强校验, 不可能撞这些.

**正修参考**: [`design_validator.py`](../../../src/omnicompany/packages/services/_core/team_builder/workers/design_validator.py) `SubmitDesignReportRouter` + `_DesignValidatorExtractResult`. 跟 [`material_id_agent.py`](../../../src/omnicompany/packages/services/_diagnosis/mass_materialization/agents/material_id_agent.py) `SubmitMaterialIdProposalsRouter` 同模式.
...
- **[information_sufficiency.md](information_sufficiency.md)** — 信息充分性的**操作手册**（F-14/Dry Run/Agent Retry 等机制）。本文是其思想基础。
- **[format.md](format.md)** — Format 设计时同样适用原则 1：不要在 Format description 里写"LLM 没验证过的规则"。
- **[pipeline.md](pipeline.md)** — 节点拓扑决策也适用：`LLMRouter` vs `Router` 的选择应按原则 1 默认偏向前者，除非确定性是铁律。
...
- 100% 必做字段 → schema `required` + `minLength` (按 [feedback_100pct_required_goes_to_skeleton](../../../C:/Users/user/.claude/projects/e--workspace/memory/feedback_100pct_required_goes_to_skeleton.md))

### AgentNodeLoop 集成

按 [`packages/services/publishing_commons/submit_artifact_tool.py`](../../src/omnicompany/packages/services/publishing_commons/submit_artifact_tool.py) 范式:
...
> **状态**: 由 gameplay_system UX figma → Unity prefab 管线驱动 (2026-04-30 L1 立)
...
### 典型案例 — figma 树 ≠ Unity prefab 树 (gameplay_system UX 域 2026-04-30)
...
2. **中信号 (需 LLM 起手 + 规模化反推)**: figma 图片节点 → Unity Sprite GUID, 走 LLM 看 figma 视觉 + scm sprite 库匹配, 多样本验证后再固化
